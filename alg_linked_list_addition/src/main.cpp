...
template<>
void Test(const Node& expected, const Node& actual)
{
    if (expected == actual)
    {
        std::cout << "\033[32mPASS\033[0m" << std::endl;
        std::cout << expected.PrintToString() << " == " << actual.PrintToString() << std::endl;
    }
    else
    {
        std::cout << "\033[31mFAIL\033[0m" << std::endl;
...
Node Add(Node* const x, Node* const y)
{
    x->Print();
    y->Print();
    return Node(3);
...
int main()
{
    ...
    Node* x = (new Node(3))->Add(1)->Add(5); // 513
    Node* y = (new Node(5))->Add(9)->Add(2); // 295
